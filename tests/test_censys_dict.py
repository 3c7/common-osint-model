...
from tests.mocks import CENSYS_HOST_JSON_LEGACY


def test_quad_one_file_success():
    file = "test_data/1.1.1.1_censys.json"
    with open(file, "r") as censys_file:
        host = Host.from_censys(json.loads(censys_file.read()))
        assert host.ip == "1.1.1.1"
        assert host.services and isinstance(host.services, list)
        assert host.services[2].tls
        assert host.services[2].tls.ja3s
        assert host.services[2].tls.ja3s == "d75f9129bb5d05492a65ff78e081bcb2"
        assert "Content-Type" in host.services[1].http.headers.keys()
        assert (
            host.services[1].http.headers.get("Location")
            == "https://1.1.1.1/application/notifications_redirections"
        )
...
def test_host_mock_success():
    ...
    assert host.services[0].dns.recursive


def test_quad_nine_file_success():
    file = "test_data/9.9.9.9_censys_v2.json"
    with open(file, "r") as censys_file:
        host = Host.from_censys(json.loads(censys_file.read()))
...
def test_quad_one_fail_be():
    file = "test_data/1.1.1.1_binaryedge.json"
...
def test_quad_one_fail_sh():
    file = "test_data/1.1.1.1_shodan.json"
